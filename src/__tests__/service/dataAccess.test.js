...
test('when has params build target url', () => {
    const {orgIdStr, eventTypeIds, eventsLimit, eventsLang, featured, eventContext} = buildTargetUrlData()
    expect(targetUrlFactory({orgIdStr, eventTypeIds, eventsLimit, eventsLang, featured, eventContext}))
        .toBe(`https://events.brahmakumaris.org/bkregistration/organisationEventReportController.do?orgEventTemplate=jsonEventExport.ftl&orgId=${orgIdStr}&eventTypeIds=${eventTypeIds}&fromIndex=0&toIndex=${eventsLimit}&mimeType=application/json`)
})
