...
export const EventCountdownContextProvider = (props) => {
    const [stateCountdownData, dispatchCountdown] = useReducer(
        reducer, {
            data: null,
            started: false
        }
    ...
    return (
        <EventCountdownContext.Provider value={{
            stateCountdownData, dispatchCountdown,
        }}>{props.children}</EventCountdownContext.Provider>
    )
}