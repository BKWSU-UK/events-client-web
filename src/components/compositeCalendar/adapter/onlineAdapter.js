...
export const updateOnlineStatus = (stateDate, eventContext) => {
    eventContext.eventsConfig[DISPLAY_ONLINE_FILTER] = true
    eventContext.filterState = {}
    if(stateDate.onlineStatus === ONLINE_STATUS.ONLINE) {
        eventContext.filterState.onlineStatus = ONLINE_STATUSES.HAS_WEBCAST
    } else if(stateDate.onlineStatus === ONLINE_STATUS.IN_PERSON) {
