...
export class SingleDateQueryAdapter {
  ...
  createQueryKey = (stateCalendar) => {
    ...
    return `${dateToKey(date)}-${onlineStatusAdapter(
      stateCalendar)}-category_${stateCalendar.categoryFilter}`
  }
  ...
  callEventList = (stateCalendar, eventContext) => {
    ...
    if (!!exactStartDate) {
      ...
      const eventTypeIds = eventTypeIdAdapter(stateCalendar,
  ...
  return (
    <>
      <LoadingContainer data={eventList} isLoading={isLoading}
                        error={error} noDataMessage={hideMissing &&
      [CARD_TYPEUI_VIEW.LONG_CARD, CARD_TYPEUI_VIEW.IMAGE_CARD].includes(
        stateCalendar.cardType) ? '' : 'No events found'}>
        ...
        {displayMoreLess(hasMore, 'calendar-show-more', incrementEventLimit,
          'Show more')}
        {displayMoreLess(hasLess, 'calendar-show-less', decrementEventLimit,
          'Show less')}
        {!!eventList && eventList.length === 0 &&
