...
export class SingleDayQueryAdapter extends MultiDateQueryAdapter {
  limitResults = (eventList, stateCalendar) => {
    ...
    return (
      eventList?.filter((e) => {
        ...
        return (
          parseInt(splits[0]) === year &&
          parseInt(splits[1]) === month &&
  ...
  callEventList = (stateCalendar, eventContext) => {
    ...
    if (!!startDate && !!endDate) {
      ...
      return getCombinedEventListWithGroupCount(
        params1,
        params2,
      ...
      >
        <div className="calendar-card-wrapper flex-wrap">
          {!!eventList &&
            ...
            eventList.map((ev, i) => {
              return (
                <EventDateCardWrapper ev={ev} timeFormat={timeFormat} key={i}>
                  {[CARD_TYPEUI_VIEW.LONG_CARD].includes(
                    stateCalendar.cardType,
                  ) ? (
                    <EventDateCard />
                  ) : (
