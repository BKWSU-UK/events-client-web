...
const generateMessage = (currentEvent, langCode, dateTimeFormat, eventType) => {
    const dateStr = currentEvent.dateList?.length > 0 ?
        renderDateTimeFromIso(currentEvent.dateList[0].startIso, langCode,
            dateTimeFormat) : ''
...
const SocialIcons = ({ currentEvent, buttonSize = 32 }) => {
    ...
    const generateUrl = eventsConfig.generateSocialMedialUrl ??
        ((e) =>`https://globalcooperationhouse.org/whatson-full/singleevent/${e.id}`)
    const generateHashtag = eventsConfig.generateHashtag ??
        ((_) => 'brahmakumaris')

    const shareUrl = generateUrl(currentEvent)
    const eventTypeId = currentEvent.eventTypeId
    const message = useMemo(
        () => generateMessage(currentEvent, langCode, dateTimeFormat,
    ...
    return (
        <div className="row social-icons">
            <div className="col-12">
                ...
                {' '}
                {!!eventImage && <PinterestShareButton
                    url={shareUrl}
                    media={`${eventImage}`}
