...
const HeroTile = ({data, showEventDate}) => {
    ...
    return (
        <div>
            <img className="tiles-lead"
                 src={heroImage} alt={ev.name} onClick={myShowEventDate} />
            <div className="tiles-events-body">
                <EventTypeTitle ev={ev} />
                <EventCardTitle ev={ev} showEventDate={myShowEventDate} />
                <EventExtraInfo currentEvent={ev} limit={EXTRA_INFO_LIMIT} linkify={true}/>
                ...
                <div className="tiles-more-details">
                    <a href="#" onClick={myShowEventDate}>{t('More details')}</a>
                </div>
            </div>
        </div>
    )
