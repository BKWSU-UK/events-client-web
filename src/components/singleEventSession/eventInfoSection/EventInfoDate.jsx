...
const EventInfoDate = ({ ev }) => {
    ...
    const calendaDayImage = eventsConfig.calendarDayImage ||
        '/assets/images/calendar_day.png'

    const renderDate = (isoDate) => {
        return moment(isoDate,
            'YYYY-MM-DD\'T\'hh:mm:ss').locale(langCode).format(`Do MMMM YYYY`)
    }
    ...
    return (
        <EventInfoContainer>
            <div className="calendar-flex-icon">
                <img src={calendaDayImage}
                     alt="15 April 2022"/>
            </div>
            <div>{startDate}{renderEndDate()}</div>
        </EventInfoContainer>
    )
}
