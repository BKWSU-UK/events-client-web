...
const OnlineFilter = () => {
    ...
    const onChange = (e) => {
        ...
        filterDispatch({ type: ACTIONS.CHANGE_ONLINE_STATE, payload : { onlineStatus } })
    }
        ...
        <div className="row form-group online-filter">
            <div className="col-12">
                <label className="col-form-label"
                       htmlFor={SELECT_NAME}>{t('Online State')}:</label>
            </div>
            <div className="col-12">
                <select className="form-control" id={SELECT_NAME}
                        value={filterState.onlineStatus}
                        onChange={onChange}>
                    {ONLINE_STATES.map((s, i) => {
                        return <option key={i} value={s}>{t(
                            `online_state_${s}`)}</option>
                    })}
                </select>
            </div>
        </div>
