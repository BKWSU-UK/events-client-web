...
function drawTimeArc (targetElement, timeUnits, isDiff, range) {
    ...
    if (!!targetElement) {
        ...
        const endAngle = isDiff ? (combinedUnit * 360) / range : 360 -
            ((uu + (lu / 1000)) * 360 / range)
        targetElement.querySelector('path').
            setAttribute('d',
                drawArc(centerWidth, centerHeight, radius, 0, endAngle))

        const smallTextSeconds = targetElement.querySelector('text')
        const { width: smallTextWidth, height: smallTextHeight } = smallTextSeconds.getBBox()
        ...
        const { width: smallTextUnitWidth, height: smallTextUnitHeight } = smallTextUnit.getBBox()
        smallTextUnit.setAttribute('x',
            centerWidth - smallTextUnitWidth / 2)
        smallTextUnit.setAttribute('y', centerHeight + smallTextUnitHeight)
    }
}

function animateArc (
    targetElement, range, speed, calcTimeUnit, isDiff = false) {
    ...
    return (
        <div className="row countdown-svg">
            {timeComponents.map((svgElement, i) => {
                return (
                    <svg id={svgElement.id} xmlns="http://www.w3.org/2000/svg"
                         ref={svgElement.ref}>
                        <circle cx="50" cy="50" r="50"/>
                        <text x="0" y="0" className="small-text"/>
                        <text x="0" y="0" className="small-text-unit">{t(
                            svgElement.label)}</text>
